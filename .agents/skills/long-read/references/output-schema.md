...
2. 把每条文字 ljg 的完整原稿放入附录，不限字数；
3. 相同结论只保留最锋利、证据最强的一处；
4. 只补必要连接语，不磨平原 Skill 的语气；
5. 删除能套在无关文章上的泛化句；
...
可选：对飞鱼的意义（<=50 字）
附录：独立深度分析
Evidence：原文金句、必要事实、原文链接
```

附录 h1 标题后先放 200~500 字导言（普通段落，受 100 字段落约束，可分多段），再按 Skill 顺序放各文字 ljg 完整原稿，每条用 h2 标题。
