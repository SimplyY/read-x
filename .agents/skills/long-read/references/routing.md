...
文字数量直接使用 content-scoring 的 `scoring_result.ljg_range`；是否生成图片直接使用 `scoring_result.ljg_card`；是否调用 ChatGPT Bridge 芒格后处理直接使用 `scoring_result.chatgpt_munger_doc`。本 Skill 不复制分档，也不使用相关性改变深度。
