...
- **`final_score >=7.5`** -> long-read 全流程 -> 卡片
- **`final_score 6.0~7.5`** -> 轻量精读 -> 卡片
...
- 高质量 → **必须生成飞书文档 → 只发原群一份卡片（600-800 字摘要通知，核心内容在文档里）**
- 中等质量 → 只发原群一份卡片（400-600 字，与原文长度成正比）
- 低质量 → 只发原群一份卡片（150-300 字，一句话 + 原文链接）
